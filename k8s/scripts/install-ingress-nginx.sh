...
# Wait for at least 1 pod to be Running and Ready (simple check: READY column = "1/1")
WAIT_COUNTER=0
while [ $WAIT_COUNTER -lt 180 ]; do
  # Count pods showing "1/1" in the READY column (Running and Ready)
  READY_COUNT=$(kubectl get pods -n ingress-nginx -l app.kubernetes.io/component=controller -o wide 2>/dev/null | grep "1/1.*Running" | wc -l)
  
  if [ "$READY_COUNT" -ge 1 ]; then
    echo "✅ Controller pod is Running and Ready (1/1)"
    break
  fi
  ...
  if [ $((WAIT_COUNTER % 30)) -eq 0 ] && [ $WAIT_COUNTER -gt 0 ]; then
    ...
    kubectl get pods -n ingress-nginx -l app.kubernetes.io/component=controller -o wide 2>/dev/null || echo "   (Pod info temporarily unavailable)"
  fi
