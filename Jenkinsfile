            ...
            steps {
                script {
                    withCredentials([
                        ...
                        // Create setup_env.sh script locally on Windows Jenkins (fixed escaping)
                        bat '''
                        ...
                        // Create MONGO_URL line using PowerShell to avoid batch escaping issues
                        powershell """
                        Add-Content -Path setup_env.sh -Value "echo MONGO_URL='${env.MONGO_URL}' >> ~/devopsians.env"
                        """
                        ...
                        // Pull latest images and restart stack (fixed env loading)
                        bat '"C:\\Program Files\\Git\\usr\\bin\\ssh.exe" -i "%SSH_KEY_FILE%" -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% "cd ~ && source ~/devopsians.env && docker pull %DOCKERHUB_USERNAME%/%BACKEND_IMAGE%:latest && docker pull %DOCKERHUB_USERNAME%/%FRONTEND_IMAGE%:latest && docker compose -f ~/docker-compose.yml down || true && DOCKERHUB_USERNAME=%DOCKERHUB_USERNAME% docker compose -f ~/docker-compose.yml up -d"'
                        ...
                        // Check if containers are running and print logs if they fail (fixed container names)
                        script {
                            def containerCheck = bat(
                                script: '"C:\\Program Files\\Git\\usr\\bin\\ssh.exe" -i "%SSH_KEY_FILE%" -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% "docker ps --format \\"{{.Names}}\\" | grep -E \\"(backend|frontend)\\""',
                                returnStatus: true
                            )
                            
                            if (containerCheck != 0) {
                                echo "WARNING: Some containers may not be running properly. Checking logs..."
                                bat '"C:\\Program Files\\Git\\usr\\bin\\ssh.exe" -i "%SSH_KEY_FILE%" -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% "echo \\"=== Backend Container Logs ===\\" && docker logs \$(docker ps -q --filter name=backend) --tail 50 2>/dev/null || echo \\"Backend container not found\\""'
                                bat '"C:\\Program Files\\Git\\usr\\bin\\ssh.exe" -i "%SSH_KEY_FILE%" -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% "echo \\"=== Frontend Container Logs ===\\" && docker logs \$(docker ps -q --filter name=frontend) --tail 50 2>/dev/null || echo \\"Frontend container not found\\""'
                            } else {
                                echo "✅ All containers are running successfully!"
                            }
                        }
                        
                        // Final health check for frontend (improved with fallbacks)
                        script {
                            def healthCheck = bat(
                                script: '"C:\\Program Files\\Git\\usr\\bin\\ssh.exe" -i "%SSH_KEY_FILE%" -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% "timeout 10 bash -c \\"</dev/tcp/localhost/80\\" 2>/dev/null || wget -q --spider http://localhost || curl -f http://localhost:80 2>/dev/null"',
                                returnStatus: true
                            )
                            ...
                            if (healthCheck == 0) {
                                ...
                                echo "⚠️ Frontend health check failed. Checking if port 80 is open..."
                                bat '"C:\\Program Files\\Git\\usr\\bin\\ssh.exe" -i "%SSH_KEY_FILE%" -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% "netstat -tlnp | grep :80 || ss -tlnp | grep :80 || echo \\"Port 80 not listening\\""'
                            }
                        }
